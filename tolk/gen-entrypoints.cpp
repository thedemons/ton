...
std::vector<var_idx_t> AuxData_OnInternalMessage_getField::generate_get_InMessage_field(CodeBlob& code,
  ...
  int idx = -1;
  if (field_name == "isBounced")
    idx = 1;
  else if (field_name == "senderAddress")
    idx = 2;
  else if (field_name == "originalForwardFee")
    idx = 3;
  else if (field_name == "createdLt")
    idx = 4;
  else if (field_name == "createdAt")
    idx = 5;
  else if (field_name == "valueCoins")
    idx = 7;
  else if (field_name == "valueExtra")
    idx = 8;
  tolk_assert(idx != -1);

  std::vector ir_msgparam = code.create_tmp_var(TypeDataInt::create(), loc, field_name.data());
  code.emplace_back(loc, Op::_Call, ir_msgparam, std::vector{code.create_int(loc, idx, "(param-idx)")},
                    lookup_function("__InMessage.getInMsgParam"));

  if (field_name == "originalForwardFee") {
    code.emplace_back(loc, Op::_Call, ir_msgparam, std::vector{ir_msgparam[0], code.create_int(loc, 0, "(basechain)")},
                      lookup_function("__InMessage.originalForwardFee"));
  }

  return ir_msgparam;
}

void handle_onInternalMessage_codegen_start(FunctionPtr f_onInternalMessage, const std::vector<var_idx_t>& rvect_params,
                                            CodeBlob& code, SrcLocation loc) {
  ...
  if (f_onBouncedMessage) {
    ...
    Op& if_isBounced = code.emplace_back(loc, Op::_If, ir_isBounced);
    {
      ...
      if (f_onBouncedMessage->is_inlined_in_place()) {
        gen_inline_fun_call_in_place(code, TypeDataVoid::create(), loc, f_onBouncedMessage, nullptr, true,
                                     {ir_bodySlice});
      } else {
    ...
    Op& op_assert = code.emplace_back(loc, Op::_Call, std::vector<var_idx_t>{}, std::move(args_throw0if),

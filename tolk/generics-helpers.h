...
struct GenericsDeclaration {
  struct ItemT {
    ...
    ItemT(std::string_view nameT, TypePtr default_type) : nameT(nameT), default_type(default_type) {
    }
  };

  explicit GenericsDeclaration(std::vector<ItemT>&& itemsT, int n_from_receiver)
      : itemsT(std::move(itemsT)), n_from_receiver(n_from_receiver) {
  }
  ...
  int size() const {
    return static_cast<int>(itemsT.size());
  }
  int find_nameT(std::string_view nameT) const;
  std::string_view get_nameT(int idx) const {
    return itemsT[idx].nameT;
  }
  TypePtr get_defaultT(int idx) const {
    return itemsT[idx].default_type;
  }
};
...
struct GenericsSubstitutions {
 ...
 public:
  explicit GenericsSubstitutions(const GenericsDeclaration* genericTs)
      : genericTs(genericTs), valuesTs(genericTs == nullptr ? 0 : genericTs->size()) {
  }
  ...
  int size() const {
    return static_cast<int>(valuesTs.size());
  }
  ...
  std::string_view nameT_at(int idx) const {
    return genericTs->get_nameT(idx);
  }
  TypePtr typeT_at(int idx) const {
    return valuesTs.at(idx);
  }

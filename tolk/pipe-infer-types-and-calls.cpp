...
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD static void fire_error_calling_asm_function_with_non1_stack_width_arg(
    FunctionPtr cur_f, SrcLocation loc, FunctionPtr fun_ref, const GenericsSubstitutions& substitutions, int arg_idx) {
  fire(cur_f, loc,
       "can not call `" + fun_ref->as_human_readable() + "` with " + to_string(substitutions.nameT_at(arg_idx)) + "=" +
           substitutions.typeT_at(arg_idx)->as_human_readable() + ", because it occupies " +
           std::to_string(substitutions.typeT_at(arg_idx)->get_width_on_stack()) + " stack slots in TVM, not 1");
}

// fire an error on `untypedTupleVar.0` when used without a hint
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD static void fire_error_cannot_deduce_untyped_tuple_access(FunctionPtr cur_f,
                                                                                                    SrcLocation loc,
                                                                                                    int index) {
  std::string idx_access = "<tuple>." + std::to_string(index);
  fire(cur_f, loc,
       "can not deduce type of `" + idx_access + "`\neither assign it to variable like `var c: int = " + idx_access +
           "` or cast the result like `" + idx_access + " as int`");
}

// fire an error on using lateinit variable before definite assignment
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD static void fire_error_using_lateinit_variable_uninitialized(
    FunctionPtr cur_f, SrcLocation loc, std::string_view name) {
...
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD static void fire_error_method_not_found(FunctionPtr cur_f, SrcLocation loc,
                                                                                  TypePtr receiver_type,
                                                                                  std::string_view method_name) {
  if (std::vector<FunctionPtr> other = lookup_methods_with_name(method_name); !other.empty()) {
    fire(cur_f, loc,
         "method `" + to_string(method_name) + "` not found for type " + to_string(receiver_type) +
  ...
  if (const Symbol* sym = lookup_global_symbol(method_name); sym && sym->try_as<FunctionPtr>()) {
    fire(cur_f, loc,
         "method `" + to_string(method_name) + "` not found, but there is a global function named `" +
             to_string(method_name) + "`\n(a function should be called `foo(arg)`, not `arg.foo()`)");
  }
...
static TypePtr try_pick_instantiated_generic_from_hint(TypePtr hint, AliasDefPtr lookup_ref) {
  ...
  if (const TypeDataGenericTypeWithTs* as_instT = lookup_ref->underlying_type->try_as<TypeDataGenericTypeWithTs>()) {
    return as_instT->struct_ref ? try_pick_instantiated_generic_from_hint(hint, as_instT->struct_ref)
...
static MethodCallCandidate choose_only_method_to_call(FunctionPtr cur_f, SrcLocation loc, TypePtr called_receiver,
...
void check_arguments_count_at_fun_call(FunctionPtr cur_f, V<ast_function_call> v, FunctionPtr called_f,
                                       AnyExprV self_obj) {
  ...
  if (!called_f->does_accept_self() && self_obj) {  // static method `Point.create(...)` called as `p.create()`
    fire(cur_f, v->loc,
         "method " + to_string(called_f) +
             " can not be called via dot\n(it's a static method, it does not accept `self`)");
  }
  if (n_max_params < n_arguments) {
    fire(cur_f, v->loc,
         "too many arguments in call to " + to_string(called_f) + ", expected " +
             std::to_string(n_max_params - delta_self) + ", have " + std::to_string(n_arguments - delta_self));
  }
  if (n_arguments < n_min_params) {
    fire(cur_f, v->loc,
         "too few arguments in call to " + to_string(called_f) + ", expected " +
             std::to_string(n_min_params - delta_self) + ", have " + std::to_string(n_arguments - delta_self));
  }
}
...
class InferTypesAndCallsAndFieldsVisitor final {
  ...
  static void process_assignment_lhs_after_infer_rhs(AnyExprV lhs, TypePtr rhs_type, FlowContext& out_flow) {
    ...
    if (auto lhs_var = lhs->try_as<ast_local_var_lhs>()) {
      TypePtr declared_type = lhs_var->marked_as_redef ? lhs_var->var_ref->declared_type
                              : lhs_var->type_node     ? lhs_var->type_node->resolved_type
                                                       : nullptr;
      ...
      TypePtr smart_casted_type = declared_type && rhs_type != TypeDataUnknown::create()
                                      ? calc_smart_cast_type_on_assignment(declared_type, rhs_type)
                                      : rhs_type;
    ...
    if (auto lhs_tensor = lhs->try_as<ast_tensor>()) {
      ...
      types_list.reserve(lhs_tensor->size());
      for (int i = 0; i < lhs_tensor->size(); ++i) {
        TypePtr ith_rhs_type =
            rhs_type_tensor && i < rhs_type_tensor->size() ? rhs_type_tensor->items[i] : TypeDataUnknown::create();
    ...
    if (auto lhs_tuple = lhs->try_as<ast_bracket_tuple>()) {
      ...
      for (int i = 0; i < lhs_tuple->size(); ++i) {
        TypePtr ith_rhs_type =
            rhs_type_tuple && i < rhs_type_tuple->size() ? rhs_type_tuple->items[i] : TypeDataUnknown::create();
  ...
  ExprFlow infer_binary_operator(V<ast_binary_operator> v, FlowContext&& flow, bool used_as_condition) {
    ...
    switch (v->tok) {
      ...
      case tok_bitwise_xor:
        flow = infer_any_expr(lhs, std::move(flow), false).out_flow;
        flow = infer_any_expr(rhs, std::move(flow), false).out_flow;
        if (lhs->inferred_type->unwrap_alias() == TypeDataBool::create() &&
            rhs->inferred_type->unwrap_alias() == TypeDataBool::create()) {
          assign_inferred_type(v, TypeDataBool::create());
        } else {
          assign_inferred_type(v, TypeDataInt::create());
        }
      ...
      case tok_logical_and: {
        ...
        if (!used_as_condition) {
          FlowContext out_flow =
              FlowContext::merge_flow(std::move(after_lhs.false_flow), std::move(after_rhs.out_flow));
        ...
        FlowContext false_flow =
            FlowContext::merge_flow(std::move(after_lhs.false_flow), std::move(after_rhs.false_flow));
  ...
  ExprFlow infer_ternary_operator(V<ast_ternary_operator> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    ExprFlow after_false =
        infer_any_expr(v->get_when_false(), std::move(after_cond.false_flow), used_as_condition, hint);
    ...
    if (branches_unifier.is_union_of_different_types()) {
      ...
      if (hint == nullptr || hint == TypeDataUnknown::create() || hint->has_genericT_inside()) {
        fire(cur_f, v->loc,
             "types of ternary branches are incompatible: " + to_string(v->get_when_true()) + " and " +
                 to_string(v->get_when_false()) +
  ...
  ExprFlow infer_is_type_operator(V<ast_is_type_operator> v, FlowContext&& flow, bool used_as_condition) {
    ...
    if (const auto* t_struct = rhs_type->try_as<TypeDataStruct>();
        t_struct && t_struct->struct_ref->is_generic_struct()) {
      // `v is Wrapper`, detect T based on type of v (`Wrapper<int> | int` => `Wrapper<int>`)
      if (TypePtr inst_rhs_type =
              try_pick_instantiated_generic_from_hint(v->get_expr()->inferred_type, t_struct->struct_ref)) {
        rhs_type = inst_rhs_type;
        v->type_node->mutate()->assign_resolved_type(rhs_type);
      } else {
        fire(cur_f, v->type_node->loc,
             "can not deduce type arguments for " + to_string(t_struct->struct_ref) + ", provide them manually");
      }
    }
    if (const auto* t_alias = rhs_type->try_as<TypeDataAlias>(); t_alias && t_alias->alias_ref->is_generic_alias()) {
      ...
      if (TypePtr inst_rhs_type =
              try_pick_instantiated_generic_from_hint(v->get_expr()->inferred_type, t_alias->alias_ref)) {
        rhs_type = inst_rhs_type;
        v->type_node->mutate()->assign_resolved_type(rhs_type);
      } else {
        fire(cur_f, v->type_node->loc,
             "can not deduce type arguments for " + to_string(t_alias->alias_ref) + ", provide them manually");
      }
    }
  ...
  ExprFlow infer_not_null_operator(V<ast_not_null_operator> v, FlowContext&& flow, bool used_as_condition) {
    ...
    TypePtr without_null_type =
        calculate_type_subtract_rhs_type(expr_type->unwrap_alias(), TypeDataNullLiteral::create());
  ...
  ExprFlow infer_parenthesized(V<ast_parenthesized_expression> v, FlowContext&& flow, bool used_as_condition,
                               TypePtr hint) {
  ...
  ExprFlow infer_braced_expression(V<ast_braced_expression> v, FlowContext&& flow, bool used_as_condition,
    ...
    assign_inferred_type(v, flow.is_unreachable() ? TypeDataNever::create()
                            : implicit_result     ? implicit_result->inferred_type
                                                  : TypeDataVoid::create());
    return ExprFlow(std::move(flow), used_as_condition);
  }
  ...
  std::vector<TypePtr> collect_type_arguments_for_fun(SrcLocation loc, const GenericsDeclaration* genericTs,
                                                      V<ast_instantiationT_list> instantiationT_list) const {
    ...
    if (instantiationT_list->size() != genericTs->size() - genericTs->n_from_receiver) {
      fire(cur_f, loc,
           "expected " + std::to_string(genericTs->size() - genericTs->n_from_receiver) + " type arguments, got " +
               std::to_string(instantiationT_list->size()));
    }
  ...
  FunctionPtr check_and_instantiate_generic_function(SrcLocation loc, FunctionPtr fun_ref,
                                                     GenericsSubstitutions&& substitutedTs) const {
  ...
  ExprFlow infer_reference(V<ast_reference> v, FlowContext&& flow, bool used_as_condition, TypePtr hint,
                           FunctionPtr* out_f_called = nullptr) const {
    ...
    if (LocalVarPtr var_ref = v->sym->try_as<LocalVarPtr>()) {
      ...
      if (fun_ref->is_generic_function() && !v_instantiationTs && !out_f_called) {
        ...
        substitutedTs.provide_type_arguments(
            collect_type_arguments_for_fun(v->loc, fun_ref->genericTs, v_instantiationTs));
  ...
  ExprFlow infer_dot_access(V<ast_dot_access> v, FlowContext&& flow, bool used_as_condition, TypePtr hint,
                            FunctionPtr* out_f_called = nullptr, AnyExprV* out_dot_obj = nullptr) {
    ...
    FunctionPtr fun_ref =
        nullptr;  // to be filled for `<any_expr>.method` / `Point.create` (both standalone or in a call)
    ...
    if (auto obj_ref = v->get_obj()->try_as<ast_reference>()) {
      if (const auto* obj_as_type = obj_ref->sym->try_as<const TypeReferenceUsedAsSymbol*>()) {
        ...
        std::tie(fun_ref, substitutedTs) =
            choose_only_method_to_call(cur_f, v_ident->loc, receiver_type, v->get_field_name());
        if (!fun_ref) {
          fire(cur_f, v_ident->loc,
               "method `" + to_string(v->get_field_name()) + "` not found for type " + to_string(receiver_type));
        }
      }
    }
    ...
    if (!fun_ref) {
      std::tie(fun_ref, substitutedTs) =
          choose_only_method_to_call(cur_f, dot_obj->loc, dot_obj->inferred_type, field_name);
    }
    ...
    if (!fun_ref) {
      ...
      if (const TypeDataUnion* as_union = obj_type->try_as<TypeDataUnion>(); as_union && as_union->or_null) {
        if (const TypeDataStruct* n_struct = as_union->or_null->try_as<TypeDataStruct>();
            n_struct && n_struct->struct_ref->find_field(field_name)) {
          fire(cur_f, v_ident->loc,
               "can not access field `" + static_cast<std::string>(field_name) + "` of a possibly nullable object " +
      ...
      if (out_f_called) {
        ...
        fire(cur_f, v_ident->loc,
             "field `" + static_cast<std::string>(field_name) + "` doesn't exist in type " + to_string(dot_obj));
      }
    }
  ...
  ExprFlow infer_function_call(V<ast_function_call> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    if (callee->kind == ast_reference) {
      ...
      flow =
          infer_dot_access(callee->as<ast_dot_access>(), std::move(flow), false, nullptr, &fun_ref, &self_obj).out_flow;
    } else {
    ...
    if (!fun_ref) {
      ...
      if (v->get_num_args() != static_cast<int>(f_callable->params_types.size())) {
        fire(cur_f, v->loc,
             "expected " + std::to_string(f_callable->params_types.size()) + " arguments, got " +
                 std::to_string(v->get_arg_list()->size()));
      }
    ...
    if (!self_obj && fun_ref->receiver_type && fun_ref->receiver_type->has_genericT_inside() &&
        callee->kind == ast_dot_access && callee->as<ast_dot_access>()->get_obj()->kind == ast_reference) {
      const auto* t_static =
          callee->as<ast_dot_access>()->get_obj()->as<ast_reference>()->sym->try_as<const TypeReferenceUsedAsSymbol*>();
  ...
  ExprFlow infer_tensor(V<ast_tensor> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    for (int i = 0; i < v->size(); ++i) {
      ...
      flow = infer_any_expr(item, std::move(flow), false,
                            tensor_hint && i < tensor_hint->size() ? tensor_hint->items[i] : nullptr)
                 .out_flow;
  ...
  ExprFlow infer_typed_tuple(V<ast_bracket_tuple> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    for (int i = 0; i < v->size(); ++i) {
      AnyExprV item = v->get_item(i);
      flow = infer_any_expr(item, std::move(flow), false,
                            tuple_hint && i < tuple_hint->size() ? tuple_hint->items[i] : nullptr)
                 .out_flow;
      types_list.emplace_back(item->inferred_type);
    }
  ...
  ExprFlow infer_match_expression(V<ast_match_expression> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    for (int i = 0; i < v->get_arms_count(); ++i) {
      ...
      FlowContext arm_flow =
          infer_any_expr(v_arm->get_pattern_expr(), arms_entry_facts.clone(), false, nullptr).out_flow;
      if (s_expr && v_arm->pattern_kind == MatchArmKind::exact_type) {
        TypePtr exact_type = v_arm->pattern_type_node->resolved_type;
        if (const auto* t_struct = exact_type->try_as<TypeDataStruct>();
            t_struct && t_struct->struct_ref->is_generic_struct()) {
          // `Wrapper => ...`, detect T based on type of subject (`Wrapper<int> | int` => `Wrapper<int>`)
          if (TypePtr inst_exact_type =
                  try_pick_instantiated_generic_from_hint(v->get_subject()->inferred_type, t_struct->struct_ref)) {
            exact_type = inst_exact_type;
            v_arm->pattern_type_node->mutate()->assign_resolved_type(exact_type);
          } else {
            fire(cur_f, v_arm->loc,
                 "can not deduce type arguments for " + to_string(t_struct->struct_ref) + ", provide them manually");
          }
        }
        if (const auto* t_alias = exact_type->try_as<TypeDataAlias>();
            t_alias && t_alias->alias_ref->is_generic_alias()) {
          // `WrapperAlias => ...`, detect T similar to structures
          if (TypePtr inst_exact_type =
                  try_pick_instantiated_generic_from_hint(v->get_subject()->inferred_type, t_alias->alias_ref)) {
            exact_type = inst_exact_type;
            v_arm->pattern_type_node->mutate()->assign_resolved_type(exact_type);
          } else {
            fire(cur_f, v_arm->loc,
      ...
      match_out_flow =
          i == 0 ? std::move(arm_flow) : FlowContext::merge_flow(std::move(match_out_flow), std::move(arm_flow));
    ...
    if (v->is_statement()) {
      ...
      if (branches_unifier.is_union_of_different_types()) {
        ...
        if (hint == nullptr || hint == TypeDataUnknown::create() || hint->has_genericT_inside()) {
          fire(cur_f, v->loc,
               "type of `match` was inferred as " + to_string(branches_unifier.get_result()) +
                   "; probably, it's not what you expected\nassign it to a variable `var v: <type> = match (...) { ... "
                   "}` manually");
        }
      }
      assign_inferred_type(v, branches_unifier.get_result());
  ...
  ExprFlow infer_object_literal(V<ast_object_literal> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    if (!struct_ref && hint) {
      ...
      if (const TypeDataUnion* hint_union = hint->unwrap_alias()->try_as<TypeDataUnion>()) {
        ...
        for (TypePtr variant : hint_union->variants) {
          ...
          if (const TypeDataGenericTypeWithTs* hint_withTs =
                  variant->unwrap_alias()->try_as<TypeDataGenericTypeWithTs>()) {
    ...
    for (StructFieldPtr field_ref : struct_ref->fields) {
      if (!(occurred_mask & (1ULL << field_ref->field_idx))) {
        ...
        if (!allow_missing) {
          fire(cur_f, v->get_body()->loc,
               "field `" + field_ref->name + "` missed in initialization of struct " + to_string(struct_ref));
        }
      }
    }
  ...
  FlowContext process_block_statement(V<ast_block_statement> v, FlowContext&& flow) {
    ...
    for (AnyV item : v->get_items()) {
      if (flow.is_unreachable() && !initially_unreachable && !v->first_unreachable &&
          item->kind != ast_empty_statement) {
 ...
 public:
  static void assign_fun_full_type(FunctionPtr fun_ref, TypePtr inferred_return_type) {
    ...
    assign_inferred_type(fun_ref, inferred_return_type,
                         TypeDataFunCallable::create(std::move(params_types), inferred_return_type));
  }
  ...
  void start_visiting_function(FunctionPtr fun_ref, V<ast_function_declaration> v_function) {
    ...
    if (fun_ref->is_code_function()) {
      ...
      if (!body_end.is_unreachable()) {
        ...
        if (fun_ref->declared_return_type ==
            TypeDataNever::create()) {  // `never` can only be declared, it can't be inferred
          fire(fun_ref, v_function->get_body()->as<ast_block_statement>()->loc_end,
               "a function returning `never` can not have a reachable endpoint");
        }
      }
      ...
      if (!fun_ref->declared_return_type) {
        ...
        if (return_unifier.is_union_of_different_types()) {
          ...
          fire(fun_ref, v_function->get_body()->loc,
               "function " + to_string(fun_ref) + " calculated return type is " + to_string(inferred_return_type) +
    ...
    for (int i = 0; i < fun_ref->get_num_params(); ++i) {
      ...
      if (param_ref->has_default_value()) {
        params_flow =
            infer_any_expr(param_ref->default_value, std::move(params_flow), false, param_ref->declared_type).out_flow;
      }
  ...
  void start_visiting_constant(GlobalConstPtr const_ref) {
    ...
    const_ref->mutate()->assign_inferred_type(const_ref->declared_type == nullptr ? const_ref->init_value->inferred_type
                                                                                  : const_ref->declared_type);
  }
...
static void infer_and_save_return_type_of_function(FunctionPtr fun_ref) {
  ...
  if (contains) {
    fire(fun_ref, fun_ref->loc,
         "could not infer return type of " + to_string(fun_ref) +
             ", because it appears in a recursive call chain\ndeclare `fun (...): <return_type>` manually");
  }
...
static void infer_and_save_type_of_constant(GlobalConstPtr const_ref) {
  ...
  if (contains) {
    throw ParseError(const_ref->loc,
                     "const `" + const_ref->name + "` appears, directly or indirectly, in its own initializer");
  }

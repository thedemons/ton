...
struct Symbol {
  ...
  Symbol(std::string name, SrcLocation loc) : name(std::move(name)), loc(loc) {
  }
...
struct LocalVarData final : Symbol {
  ...
  LocalVarData(std::string name, SrcLocation loc, TypePtr declared_type, AnyExprV default_value, int flags,
               int param_idx)
      : Symbol(std::move(name), loc)
  ...
  bool is_lateinit() const {
    return flags & flagLateInit;
  }
  bool is_mutate_parameter() const {
    return flags & flagMutateParameter;
  }
  bool is_used_as_lval() const {
    return flags & flagUsedAsLVal;
  }
  bool has_default_value() const {
    return default_value != nullptr;
  }

  LocalVarData* mutate() const {
    return const_cast<LocalVarData*>(this);
  }
...
typedef std::variant<FunctionBodyCode*, FunctionBodyAsm*, FunctionBodyBuiltin*> FunctionBody;

struct FunctionData final : Symbol {
  static constexpr int EMPTY_TVM_METHOD_ID = -10;

  enum {
    flagTypeInferringDone =
        4,  // type inferring step of function's body (all AST nodes assigning v->inferred_type) is done
    flagUsedAsNonCall =
        8,  // used not only as `f()`, but as a 1-st class function (assigned to var, pushed to tuple, etc.)
    flagMarkedAsPure =
        16,  // declared as `pure`, can't call impure and access globals, unused invocations are optimized out
    ...
    flagReallyUsed =
        2048,  // calculated via dfs from used functions; declared but unused functions are not codegenerated
    flagCompileTimeVal =
        4096,  // calculated only at compile-time for constant arguments: `ton("0.05")`, `stringCrc32`, and others
  ...
  FunctionData(std::string name, SrcLocation loc, std::string method_name, AnyTypeV receiver_type_node,
               AnyTypeV return_type_node, std::vector<LocalVarData> parameters, int initial_flags,
               FunctionInlineMode inline_mode, const GenericsDeclaration* genericTs,
               const GenericsSubstitutions* substitutedTs, FunctionBody body, AnyV ast_root)
      : Symbol(std::move(name), loc)
      , flags(initial_flags)
  ...
  FunctionData(std::string name, SrcLocation loc, std::string method_name, TypePtr receiver_type,
               TypePtr declared_return_type, std::vector<LocalVarData> parameters, int initial_flags,
               FunctionInlineMode inline_mode, const GenericsDeclaration* genericTs,
               const GenericsSubstitutions* substitutedTs, FunctionBody body, AnyV ast_root)
      : Symbol(std::move(name), loc)
      , flags(initial_flags)
  ...
  const LocalVarData& get_param(int idx) const {
    return parameters[idx];
  }
  LocalVarPtr find_param(std::string_view name) const;

  bool is_code_function() const {
    return std::holds_alternative<FunctionBodyCode*>(body);
  }
  bool is_asm_function() const {
    return std::holds_alternative<FunctionBodyAsm*>(body);
  }
  bool is_builtin_function() const {
    return ast_root == nullptr;
  }
  bool is_method() const {
    return !method_name.empty();
  }
  bool is_static_method() const {
    return is_method() && !does_accept_self();
  }
  ...
  bool is_instantiation_of_generic_function() const {
    return substitutedTs != nullptr;
  }

  bool is_inlined_in_place() const {
    return inline_mode == FunctionInlineMode::inlineInPlace;
  }
  bool is_type_inferring_done() const {
    return flags & flagTypeInferringDone;
  }
  bool is_used_as_noncall() const {
    return flags & flagUsedAsNonCall;
  }
  bool is_marked_as_pure() const {
    return flags & flagMarkedAsPure;
  }
  bool is_implicit_return() const {
    return flags & flagImplicitReturn;
  }
  bool is_contract_getter() const {
    return flags & flagContractGetter;
  }
  bool has_tvm_method_id() const {
    return tvm_method_id != EMPTY_TVM_METHOD_ID;
  }
  ...
  bool does_mutate_self() const {
    return (flags & flagAcceptsSelf) && parameters[0].is_mutate_parameter();
  }
  bool is_really_used() const {
    return flags & flagReallyUsed;
  }
  bool is_compile_time_const_val() const {
    return flags & flagCompileTimeVal;
  }
  bool is_compile_time_special_gen() const {
    return flags & flagCompileTimeGen;
  }
  bool is_variadic_width_T_allowed() const {
    return flags & flagAllowAnyWidthT;
  }
  bool is_manual_on_bounce() const {
    return flags & flagManualOnBounce;
  }

  bool does_need_codegen() const;

  FunctionData* mutate() const {
    return const_cast<FunctionData*>(this);
  }
...
struct GlobalVarData final : Symbol {
  ...
  GlobalVarData(std::string name, SrcLocation loc, AnyTypeV type_node)
      : Symbol(std::move(name), loc), type_node(type_node) {
  }
  ...
  GlobalVarData* mutate() const {
    return const_cast<GlobalVarData*>(this);
  }
  void assign_resolved_type(TypePtr declared_type);
  void assign_is_really_used();
};
...
struct GlobalConstData final : Symbol {
  ...
  GlobalConstData(std::string name, SrcLocation loc, AnyTypeV type_node, AnyExprV init_value)
      : Symbol(std::move(name), loc), type_node(type_node), init_value(init_value) {
  }

  GlobalConstData* mutate() const {
    return const_cast<GlobalConstData*>(this);
  }
  void assign_resolved_type(TypePtr declared_type);
  void assign_inferred_type(TypePtr inferred_type);
...
struct AliasDefData final : Symbol {
  ...
  AliasDefData(std::string name, SrcLocation loc, AnyTypeV underlying_type_node, const GenericsDeclaration* genericTs,
               const GenericsSubstitutions* substitutedTs, AnyV ast_root)
      : Symbol(std::move(name), loc)
  ...
  bool is_generic_alias() const {
    return genericTs != nullptr;
  }
  bool is_instantiation_of_generic_alias() const {
    return substitutedTs != nullptr;
  }

  AliasDefData* mutate() const {
    return const_cast<AliasDefData*>(this);
  }
  void assign_resolved_genericTs(const GenericsDeclaration* genericTs);
...
struct StructFieldData final : Symbol {
  ...
  StructFieldData* mutate() const {
    return const_cast<StructFieldData*>(this);
  }
  void assign_resolved_type(TypePtr declared_type);
  void assign_default_value(AnyExprV default_value);

  StructFieldData(std::string name, SrcLocation loc, int field_idx, AnyTypeV type_node, AnyExprV default_value)
      : Symbol(std::move(name), loc), field_idx(field_idx), type_node(type_node), default_value(default_value) {
  }
};
...
struct StructData final : Symbol {
  ...
  struct PackOpcode {
    ...
    PackOpcode(int64_t pack_prefix, int prefix_len) : pack_prefix(pack_prefix), prefix_len(prefix_len) {
    }
  ...
  StructFieldPtr get_field(int i) const {
    return fields.at(i);
  }
  StructFieldPtr find_field(std::string_view field_name) const;

  bool is_generic_struct() const {
    return genericTs != nullptr;
  }
  bool is_instantiation_of_generic_struct() const {
    return substitutedTs != nullptr;
  }

  StructData* mutate() const {
    return const_cast<StructData*>(this);
  }
  void assign_resolved_genericTs(const GenericsDeclaration* genericTs);

  StructData(std::string name, SrcLocation loc, std::vector<StructFieldPtr>&& fields, PackOpcode opcode,
             Overflow1023Policy overflow1023_policy, const GenericsDeclaration* genericTs,
             const GenericsSubstitutions* substitutedTs, AnyV ast_root)
      : Symbol(std::move(name), loc)
...
struct TypeReferenceUsedAsSymbol final : Symbol {
  ...
  TypeReferenceUsedAsSymbol(std::string name, SrcLocation loc, TypePtr resolved_type)
      : Symbol(std::move(name), loc), resolved_type(resolved_type) {
  }
};

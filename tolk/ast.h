...
struct ASTNodeDeclaredTypeBase : ASTNodeBase {
  ...
  ASTNodeDeclaredTypeBase* mutate() const {
    return const_cast<ASTNodeDeclaredTypeBase*>(this);
  }
...
struct ASTNodeExpressionBase : ASTNodeBase {
  ...
  ASTNodeExpressionBase* mutate() const {
    return const_cast<ASTNodeExpressionBase*>(this);
  }
...
struct ASTTypeLeaf : ASTNodeDeclaredTypeBase {
 protected:
  ASTTypeLeaf(ASTNodeKind kind, SrcLocation loc) : ASTNodeDeclaredTypeBase(kind, loc) {
  }
};
...
struct ASTExprLeaf : ASTNodeExpressionBase {
 ...
 protected:
  ASTExprLeaf(ASTNodeKind kind, SrcLocation loc) : ASTNodeExpressionBase(kind, loc) {
  }
};
...
struct ASTExprUnary : ASTNodeExpressionBase {
 ...
 protected:
  ...
  ASTExprUnary(ASTNodeKind kind, SrcLocation loc, AnyExprV child) : ASTNodeExpressionBase(kind, loc), child(child) {
  }
};
...
struct ASTStatementUnary : ASTNodeStatementBase {
 ...
 protected:
  ...
  AnyExprV child_as_expr() const {
    return reinterpret_cast<AnyExprV>(child);
  }

  ASTStatementUnary(ASTNodeKind kind, SrcLocation loc, AnyV child) : ASTNodeStatementBase(kind, loc), child(child) {
  }
};
...
struct ASTStatementVararg : ASTNodeStatementBase {
 ...
 protected:
  ...
  AnyExprV child_as_expr(int i) const {
    return reinterpret_cast<AnyExprV>(children.at(i));
  }
...
struct ASTOtherLeaf : ASTNodeBase {
 ...
 protected:
  ASTOtherLeaf(ASTNodeKind kind, SrcLocation loc) : ASTNodeBase(kind, loc) {
  }
};
...
struct Vertex<ast_identifier> final : ASTOtherLeaf {
  ...
  Vertex(SrcLocation loc, std::string_view name) : ASTOtherLeaf(ast_identifier, loc), name(name) {
  }
};
...
struct Vertex<ast_type_leaf_text> final : ASTTypeLeaf {
  ...
  Vertex(SrcLocation loc, std::string_view text) : ASTTypeLeaf(ast_type_leaf_text, loc), text(text) {
  }
};
...
struct Vertex<ast_type_question_nullable> final : ASTTypeVararg {
  AnyTypeV get_inner() const {
    return children.at(0);
  }

  Vertex(SrcLocation loc, AnyTypeV inner) : ASTTypeVararg(ast_type_question_nullable, loc, {inner}) {
  }
};
...
struct Vertex<ast_type_parenthesis_tensor> final : ASTTypeVararg {
  const std::vector<AnyTypeV>& get_items() const {
    return children;
  }
...
struct Vertex<ast_type_arrow_callable> final : ASTTypeVararg {
  const std::vector<AnyTypeV>& get_params_and_return() const {
    return children;
  }
...
struct Vertex<ast_type_vertical_bar_union> final : ASTTypeVararg {
  const std::vector<AnyTypeV>& get_variants() const {
    return children;
  }
...
struct Vertex<ast_type_triangle_args> final : ASTTypeVararg {
  const std::vector<AnyTypeV>& get_inner_and_args() const {
    return children;
  }
...
struct Vertex<ast_empty_expression> final : ASTExprLeaf {
  explicit Vertex(SrcLocation loc) : ASTExprLeaf(ast_empty_expression, loc) {
  }
};
...
struct Vertex<ast_parenthesized_expression> final : ASTExprUnary {
  AnyExprV get_expr() const {
    return child;
  }

  Vertex(SrcLocation loc, AnyExprV expr) : ASTExprUnary(ast_parenthesized_expression, loc, expr) {
  }
};
...
struct Vertex<ast_braced_expression> final : ASTExprBlockOfStatements {
  auto get_block_statement() const {
    return child_block_statement->as<ast_block_statement>();
  }
...
struct Vertex<ast_braced_yield_result> final : ASTExprUnary {
  ...
  Vertex(SrcLocation loc, AnyExprV expr) : ASTExprUnary(ast_braced_yield_result, loc, expr) {
  }
};
...
struct Vertex<ast_artificial_aux_vertex> final : ASTExprUnary {
  ...
  AnyExprV get_wrapped_expr() const {
    return child;
  }

  Vertex(SrcLocation loc, AnyExprV wrapped_expr, const ASTAuxData* aux_data, TypePtr inferred_type)
      : ASTExprUnary(ast_artificial_aux_vertex, loc, wrapped_expr), aux_data(aux_data) {
...
struct Vertex<ast_tensor> final : ASTExprVararg {
  const std::vector<AnyExprV>& get_items() const {
    return children;
  }
  AnyExprV get_item(int i) const {
    return child(i);
  }

  Vertex(SrcLocation loc, std::vector<AnyExprV> items) : ASTExprVararg(ast_tensor, loc, std::move(items)) {
  }
};
...
struct Vertex<ast_bracket_tuple> final : ASTExprVararg {
  ...
  Vertex(SrcLocation loc, std::vector<AnyExprV> items) : ASTExprVararg(ast_bracket_tuple, loc, std::move(items)) {
  }
};
...
struct Vertex<ast_reference> final : ASTExprLeaf {
 ...
 public:
  ...
  bool has_instantiationTs() const {
    return instantiationTs != nullptr;
  }
  auto get_instantiationTs() const {
  ...
  Vertex(SrcLocation loc, V<ast_identifier> name_identifier, V<ast_instantiationT_list> instantiationTs)
      : ASTExprLeaf(ast_reference, loc), identifier(name_identifier), instantiationTs(instantiationTs) {
  }
};
...
struct Vertex<ast_local_var_lhs> final : ASTExprLeaf {
 ...
 public:
  LocalVarPtr var_ref =
      nullptr;  // filled on resolve identifiers; for `redef` points to declared above; for underscore, name is empty
  ...
  V<ast_identifier> get_identifier() const {
    return identifier;
  }
  std::string_view get_name() const {
    return identifier->name;
  }  // empty for underscore
  ...
  Vertex(SrcLocation loc, V<ast_identifier> identifier, AnyTypeV type_node, bool is_immutable, bool is_lateinit,
         bool marked_as_redef)
      : ASTExprLeaf(ast_local_var_lhs, loc)
      , identifier(identifier)
      , type_node(type_node)
      , is_immutable(is_immutable)
      , is_lateinit(is_lateinit)
      , marked_as_redef(marked_as_redef) {
  }
};
...
struct Vertex<ast_local_vars_declaration> final : ASTExprUnary {
  AnyExprV get_expr() const {
    return child;
  }  // ast_local_var_lhs / ast_tensor / ast_bracket_tuple

  Vertex(SrcLocation loc, AnyExprV expr) : ASTExprUnary(ast_local_vars_declaration, loc, expr) {
  }
};
...
struct Vertex<ast_int_const> final : ASTExprLeaf {
  ...
  Vertex(SrcLocation loc, td::RefInt256 intval, std::string_view orig_str)
      : ASTExprLeaf(ast_int_const, loc), intval(std::move(intval)), orig_str(orig_str) {
  }
};
...
struct Vertex<ast_string_const> final : ASTExprLeaf {
  ...
  Vertex(SrcLocation loc, std::string_view str_val) : ASTExprLeaf(ast_string_const, loc), str_val(str_val) {
  }
};
...
struct Vertex<ast_bool_const> final : ASTExprLeaf {
  ...
  Vertex(SrcLocation loc, bool bool_val) : ASTExprLeaf(ast_bool_const, loc), bool_val(bool_val) {
  }
};
...
struct Vertex<ast_null_keyword> final : ASTExprLeaf {
  explicit Vertex(SrcLocation loc) : ASTExprLeaf(ast_null_keyword, loc) {
  }
};
...
struct Vertex<ast_argument> final : ASTExprUnary {
  ...
  Vertex(SrcLocation loc, AnyExprV expr, bool passed_as_mutate)
      : ASTExprUnary(ast_argument, loc, expr), passed_as_mutate(passed_as_mutate) {
  }
};
...
struct Vertex<ast_argument_list> final : ASTExprVararg {
  const std::vector<AnyExprV>& get_arguments() const {
    return children;
  }
  auto get_arg(int i) const {
    return child(i)->as<ast_argument>();
  }
...
struct Vertex<ast_dot_access> final : ASTExprUnary {
 ...
 public:
  typedef std::variant<FunctionPtr,     // for `t.tupleAt` target is `tupleAt` global function
  ...
  bool is_target_fun_ref() const {
    return std::holds_alternative<FunctionPtr>(target);
  }
  bool is_target_struct_field() const {
    return std::holds_alternative<StructFieldPtr>(target);
  }
  bool is_target_indexed_access() const {
    return std::holds_alternative<int>(target);
  }

  AnyExprV get_obj() const {
    return child;
  }
  auto get_identifier() const {
    return identifier;
  ...
  Vertex(SrcLocation loc, AnyExprV obj, V<ast_identifier> identifier, V<ast_instantiationT_list> instantiationTs)
      : ASTExprUnary(ast_dot_access, loc, obj), identifier(identifier), instantiationTs(instantiationTs) {
  }
};
...
struct Vertex<ast_function_call> final : ASTExprBinary {
  FunctionPtr fun_maybe =
      nullptr;  // filled while type inferring for `globalF()` / `obj.f()`; remains nullptr for `local_var()` / `getF()()`
  bool dot_obj_is_self =
      false;  // true for `obj.method()` (obj will be `self` in method); false for `globalF()` / `Point.create()`

  AnyExprV get_callee() const {
    return lhs;
  }
  AnyExprV get_self_obj() const {
    return dot_obj_is_self ? lhs->as<ast_dot_access>()->get_obj() : nullptr;
  }
  auto get_arg_list() const {
    return rhs->as<ast_argument_list>();
  }
  int get_num_args() const {
    return rhs->as<ast_argument_list>()->size();
  }
  auto get_arg(int i) const {
    return rhs->as<ast_argument_list>()->get_arg(i);
  }
...
struct Vertex<ast_underscore> final : ASTExprLeaf {
  explicit Vertex(SrcLocation loc) : ASTExprLeaf(ast_underscore, loc) {
  }
};
...
struct Vertex<ast_assign> final : ASTExprBinary {
  AnyExprV get_lhs() const {
    return lhs;
  }
  AnyExprV get_rhs() const {
    return rhs;
  }

  explicit Vertex(SrcLocation loc, AnyExprV lhs, AnyExprV rhs) : ASTExprBinary(ast_assign, loc, lhs, rhs) {
  }
};
...
struct Vertex<ast_set_assign> final : ASTExprBinary {
  ...
  Vertex(SrcLocation loc, std::string_view operator_name, TokenType tok, AnyExprV lhs, AnyExprV rhs)
      : ASTExprBinary(ast_set_assign, loc, lhs, rhs), operator_name(operator_name), tok(tok) {
  }
};
...
struct Vertex<ast_unary_operator> final : ASTExprUnary {
  ...
  Vertex(SrcLocation loc, std::string_view operator_name, TokenType tok, AnyExprV rhs)
      : ASTExprUnary(ast_unary_operator, loc, rhs), operator_name(operator_name), tok(tok) {
  }
};
...
struct Vertex<ast_binary_operator> final : ASTExprBinary {
  ...
  Vertex(SrcLocation loc, std::string_view operator_name, TokenType tok, AnyExprV lhs, AnyExprV rhs)
      : ASTExprBinary(ast_binary_operator, loc, lhs, rhs), operator_name(operator_name), tok(tok) {
  }
};
...
struct Vertex<ast_ternary_operator> final : ASTExprVararg {
  AnyExprV get_cond() const {
    return child(0);
  }
  AnyExprV get_when_true() const {
    return child(1);
  }
  AnyExprV get_when_false() const {
    return child(2);
  }
...
struct Vertex<ast_cast_as_operator> final : ASTExprUnary {
  ...
  Vertex(SrcLocation loc, AnyExprV expr, AnyTypeV type_node)
      : ASTExprUnary(ast_cast_as_operator, loc, expr), type_node(type_node) {
  }
};
...
struct Vertex<ast_is_type_operator> final : ASTExprUnary {
  ...
  Vertex(SrcLocation loc, AnyExprV expr, AnyTypeV type_node, bool is_negated)
      : ASTExprUnary(ast_is_type_operator, loc, expr), type_node(type_node), is_negated(is_negated) {
  }
};
...
struct Vertex<ast_not_null_operator> final : ASTExprUnary {
  ...
  Vertex(SrcLocation loc, AnyExprV expr) : ASTExprUnary(ast_not_null_operator, loc, expr) {
  }
};
...
struct Vertex<ast_lazy_operator> final : ASTExprUnary {
  ...
  Vertex(SrcLocation loc, AnyExprV expr) : ASTExprUnary(ast_lazy_operator, loc, expr) {
  }
};
...
struct Vertex<ast_match_expression> final : ASTExprVararg {
  AnyExprV get_subject() const {
    return child(0);
  }
  int get_arms_count() const {
    return size() - 1;
  }
  auto get_arm(int i) const {
    return child(i + 1)->as<ast_match_arm>();
  }
  const std::vector<AnyExprV>& get_all_children() const {
    return children;
  }

  bool is_statement() const {
    return !is_rvalue && !is_lvalue;
  }
...
struct Vertex<ast_match_arm> final : ASTExprBinary {
  ...
  AnyExprV get_pattern_expr() const {
    return lhs;
  }
  auto get_body() const {
    return rhs->as<ast_braced_expression>();
  }
  ...
  Vertex(SrcLocation loc, MatchArmKind pattern_kind, AnyTypeV pattern_type_node, AnyExprV pattern_expr,
         V<ast_braced_expression> body)
      : ASTExprBinary(ast_match_arm, loc, pattern_expr, body)
      , pattern_kind(pattern_kind)
      , pattern_type_node(pattern_type_node) {
  }
};
...
struct Vertex<ast_object_field> final : ASTExprUnary {
 ...
 public:
  ...
  AnyExprV get_init_val() const {
    return child;
  }
  std::string_view get_field_name() const {
    return identifier->name;
  }
  V<ast_identifier> get_field_identifier() const {
    return identifier;
  }
  ...
  Vertex(SrcLocation loc, V<ast_identifier> name_identifier, AnyExprV init_val)
      : ASTExprUnary(ast_object_field, loc, init_val), identifier(name_identifier) {
  }
};
...
struct Vertex<ast_object_body> final : ASTExprVararg {
  ...
  auto get_field(int i) const {
    return child(i)->as<ast_object_field>();
  }
  std::vector<AnyExprV> get_all_fields() const {
    return children;
  }

  Vertex(SrcLocation loc, std::vector<AnyExprV>&& fields) : ASTExprVararg(ast_object_body, loc, std::move(fields)) {
  }
};
...
struct Vertex<ast_object_literal> final : ASTExprUnary {
  ...
  auto get_body() const {
    return child->as<ast_object_body>();
  }
  ...
  Vertex(SrcLocation loc, AnyTypeV type_node, V<ast_object_body> body)
      : ASTExprUnary(ast_object_literal, loc, body), type_node(type_node) {
  }
};
...
struct Vertex<ast_empty_statement> final : ASTStatementVararg {
  explicit Vertex(SrcLocation loc) : ASTStatementVararg(ast_empty_statement, loc, {}) {
  }
};
...
struct Vertex<ast_block_statement> final : ASTStatementVararg {
  ...
  const std::vector<AnyV>& get_items() const {
    return children;
  }
  AnyV get_item(int i) const {
    return children.at(i);
  }
  ...
  Vertex(SrcLocation loc, SrcLocation loc_end, std::vector<AnyV>&& items)
      : ASTStatementVararg(ast_block_statement, loc, std::move(items)), loc_end(loc_end) {
  }
};
...
struct Vertex<ast_return_statement> : ASTStatementUnary {
  AnyExprV get_return_value() const {
    return child_as_expr();
  }
  bool has_return_value() const {
    return child->kind != ast_empty_expression;
  }

  Vertex(SrcLocation loc, AnyExprV child) : ASTStatementUnary(ast_return_statement, loc, child) {
  }
};
...
struct Vertex<ast_if_statement> final : ASTStatementVararg {
  ...
  AnyExprV get_cond() const {
    return child_as_expr(0);
  }
  auto get_if_body() const {
    return children.at(1)->as<ast_block_statement>();
  }
  auto get_else_body() const {
    return children.at(2)->as<ast_block_statement>();
  }  // always exists (when else omitted, it's empty)

  Vertex(SrcLocation loc, bool is_ifnot, AnyExprV cond, V<ast_block_statement> if_body,
         V<ast_block_statement> else_body)
      : ASTStatementVararg(ast_if_statement, loc, {cond, if_body, else_body}), is_ifnot(is_ifnot) {
  }
};
...
struct Vertex<ast_do_while_statement> final : ASTStatementVararg {
  auto get_body() const {
    return children.at(0)->as<ast_block_statement>();
  }
  AnyExprV get_cond() const {
    return child_as_expr(1);
  }
...
struct Vertex<ast_throw_statement> final : ASTStatementVararg {
  AnyExprV get_thrown_code() const {
    return child_as_expr(0);
  }
  bool has_thrown_arg() const {
    return child_as_expr(1)->kind != ast_empty_expression;
  }
  AnyExprV get_thrown_arg() const {
    return child_as_expr(1);
  }
...
struct Vertex<ast_try_catch_statement> final : ASTStatementVararg {
  auto get_try_body() const {
    return children.at(0)->as<ast_block_statement>();
  }
  auto get_catch_expr() const {
    return children.at(1)->as<ast_tensor>();
  }  // (excNo, arg), always len 2
  auto get_catch_body() const {
    return children.at(2)->as<ast_block_statement>();
  }
...
struct Vertex<ast_asm_body> final : ASTStatementVararg {
  ...
  const std::vector<AnyV>& get_asm_commands() const {
    return children;
  }  // ast_string_const[]
  ...
  Vertex(SrcLocation loc, std::vector<int> arg_order, std::vector<int> ret_order, std::vector<AnyV> asm_commands)
      ...
      , arg_order(std::move(arg_order))
      , ret_order(std::move(ret_order)) {
  }
};
...
struct Vertex<ast_genericsT_item> final : ASTOtherLeaf {
  ...
  Vertex(SrcLocation loc, std::string_view nameT, AnyTypeV default_type_node)
      : ASTOtherLeaf(ast_genericsT_item, loc), nameT(nameT), default_type_node(default_type_node) {
  }
};
...
struct Vertex<ast_genericsT_list> final : ASTOtherVararg {
  std::vector<AnyV> get_items() const {
    return children;
  }
  auto get_item(int i) const {
    return children.at(i)->as<ast_genericsT_item>();
  }
...
struct Vertex<ast_instantiationT_item> final : ASTOtherLeaf {
  ...
  Vertex(SrcLocation loc, AnyTypeV type_node) : ASTOtherLeaf(ast_instantiationT_item, loc), type_node(type_node) {
  }
};
...
struct Vertex<ast_instantiationT_list> final : ASTOtherVararg {
  ...
  auto get_item(int i) const {
    return children.at(i)->as<ast_instantiationT_item>();
  }
...
struct Vertex<ast_parameter> final : ASTOtherLeaf {
  ...
  Vertex(SrcLocation loc, std::string_view param_name, AnyTypeV type_node, AnyExprV default_value,
         bool declared_as_mutate)
      : ASTOtherLeaf(ast_parameter, loc)
      , param_name(param_name)
      , type_node(type_node)
      , default_value(default_value)
      , declared_as_mutate(declared_as_mutate) {
  }
};
...
struct Vertex<ast_parameter_list> final : ASTOtherVararg {
  const std::vector<AnyV>& get_params() const {
    return children;
  }
  auto get_param(int i) const {
    return children.at(i)->as<ast_parameter>();
  }

  Vertex(SrcLocation loc, std::vector<AnyV> params) : ASTOtherVararg(ast_parameter_list, loc, std::move(params)) {
  }

  int lookup_idx(std::string_view param_name) const;
  int get_mutate_params_count() const;
  bool has_mutate_params() const {
    return get_mutate_params_count() > 0;
  }
};
...
struct Vertex<ast_annotation> final : ASTOtherVararg {
  ...
  auto get_arg() const {
    return children.at(0)->as<ast_tensor>();
  }
  ...
  Vertex(SrcLocation loc, std::string_view name, AnnotationKind kind, V<ast_tensor> arg_probably_empty)
      : ASTOtherVararg(ast_annotation, loc, {arg_probably_empty}), name(name), kind(kind) {
  }
};
...
struct Vertex<ast_function_declaration> final : ASTOtherVararg {
  auto get_identifier() const {
    return children.at(0)->as<ast_identifier>();
  }
  int get_num_params() const {
    return children.at(1)->as<ast_parameter_list>()->size();
  }
  auto get_param_list() const {
    return children.at(1)->as<ast_parameter_list>();
  }
  auto get_param(int i) const {
    return children.at(1)->as<ast_parameter_list>()->get_param(i);
  }
  AnyV get_body() const {
    return children.at(2);
  }  // ast_block_statement / ast_asm_body
  ...
  bool is_asm_function() const {
    return children.at(2)->kind == ast_asm_body;
  }
  bool is_code_function() const {
    return children.at(2)->kind == ast_block_statement;
  }
  bool is_builtin_function() const {
    return children.at(2)->kind == ast_empty_statement;
  }
  ...
  Vertex(SrcLocation loc, V<ast_identifier> name_identifier, V<ast_parameter_list> parameters, AnyV body,
         AnyTypeV receiver_type_node, AnyTypeV return_type_node, V<ast_genericsT_list> genericsT_list,
         td::RefInt256 tvm_method_id, int flags, FunctionInlineMode inline_mode)
      ...
      , tvm_method_id(std::move(tvm_method_id))
      , flags(flags)
...
struct Vertex<ast_global_var_declaration> final : ASTOtherVararg {
  ...
  Vertex(SrcLocation loc, V<ast_identifier> name_identifier, AnyTypeV type_node)
      : ASTOtherVararg(ast_global_var_declaration, loc, {name_identifier}), type_node(type_node) {
  }
};
...
struct Vertex<ast_constant_declaration> final : ASTOtherVararg {
  ...
  Vertex(SrcLocation loc, V<ast_identifier> name_identifier, AnyTypeV type_node, AnyExprV init_value)
      : ASTOtherVararg(ast_constant_declaration, loc, {name_identifier, init_value}), type_node(type_node) {
  }
};
...
struct Vertex<ast_type_alias_declaration> final : ASTOtherVararg {
  ...
  Vertex(SrcLocation loc, V<ast_identifier> name_identifier, V<ast_genericsT_list> genericsT_list,
...
struct Vertex<ast_struct_field> final : ASTOtherVararg {
  ...
  Vertex(SrcLocation loc, V<ast_identifier> name_identifier, AnyExprV default_value, AnyTypeV type_node)
      : ASTOtherVararg(ast_struct_field, loc, {name_identifier}), type_node(type_node), default_value(default_value) {
  }
};
...
struct Vertex<ast_struct_body> final : ASTOtherVararg {
  ...
  auto get_field(int i) const {
    return children.at(i)->as<ast_struct_field>();
  }
  const std::vector<AnyV>& get_all_fields() const {
    return children;
  }

  Vertex(SrcLocation loc, std::vector<AnyV>&& fields) : ASTOtherVararg(ast_struct_body, loc, std::move(fields)) {
  }
};
...
struct Vertex<ast_struct_declaration> final : ASTOtherVararg {
  ...
  bool has_opcode() const {
    return children.at(1)->kind != ast_empty_expression;
  }
  auto get_opcode() const {
    return children.at(1)->as<ast_int_const>();
  }
  auto get_struct_body() const {
    return children.at(2)->as<ast_struct_body>();
  }
  ...
  Vertex(SrcLocation loc, V<ast_identifier> name_identifier, V<ast_genericsT_list> genericsT_list,
         StructData::Overflow1023Policy overflow1023_policy, AnyExprV opcode, V<ast_struct_body> struct_body)
...
struct Vertex<ast_tolk_required_version> final : ASTOtherLeaf {
  ...
  Vertex(SrcLocation loc, std::string_view semver) : ASTOtherLeaf(ast_tolk_required_version, loc), semver(semver) {
  }
};
...
struct Vertex<ast_import_directive> final : ASTOtherVararg {
  ...
  auto get_file_leaf() const {
    return children.at(0)->as<ast_string_const>();
  }

  std::string get_file_name() const {
    return static_cast<std::string>(children.at(0)->as<ast_string_const>()->str_val);
  }
  ...
  Vertex(SrcLocation loc, V<ast_string_const> file_name) : ASTOtherVararg(ast_import_directive, loc, {file_name}) {
  }
};
...
struct Vertex<ast_tolk_file> final : ASTOtherVararg {
  ...
  const std::vector<AnyV>& get_toplevel_declarations() const {
    return children;
  }

  Vertex(const SrcFile* file, std::vector<AnyV> toplevel_declarations)
      : ASTOtherVararg(ast_tolk_file, SrcLocation(file), std::move(toplevel_declarations)), file(file) {
  }
};
